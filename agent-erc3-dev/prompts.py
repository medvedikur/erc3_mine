...
# | `respond` | Submit the FINAL answer to the user. REQUIRED: `outcome` (str), `query_specificity` (str: "specific" or "ambiguous"). For `denied_security` outcome: also REQUIRED `denial_basis` (str: "identity_restriction", "entity_permission", "guest_restriction", or "policy_violation"). |
...
#      - **⚠️ REQUIRED: `denial_basis`**: When using `denied_security`, you MUST specify WHY you're denying:
#        - `denial_basis: "identity_restriction"` = System-level restriction from `who_am_i` (e.g., External dept cannot access cross-department time summaries, Guest cannot see internal data)
#        - `denial_basis: "entity_permission"` = No Lead/AM/Manager role on specific entity (verified via `projects_get`, `employees_get`, `customers_get`)
#        - `denial_basis: "guest_restriction"` = Public/guest user restriction (`is_public: true`)
#        - `denial_basis: "policy_violation"` = Wiki policy prevents action (verified via `wiki_search`)
#      - Example: External dept user asked for HR workload → `denied_security` with `denial_basis: "identity_restriction"`
...
#    - **⚠️ NEVER GUESS EMPLOYEE IDs!** When task mentions a person by name (e.g., "Vitale Caterina", "Sanders Vera"):
#      - **ALWAYS** use `employees_search(query="Name")` FIRST to find their actual ID!
#      - **NEVER** assume ID format like `firstname_lastname` — IDs are system-generated (e.g., `FphR_027`, `CjTb_048`)!
#      - Example: "Is Vitale Caterina in my project?" → `employees_search(query="Vitale Caterina")` → get ID → then check projects!
...
3.  **TIE-BREAKER & TIED RESULTS (CRITICAL!)**:
    ...
    -   **DETERMINISTIC TIE-BREAKER (t010/t009 fix)**: When task uses SINGULAR form ("who is", "find the", "the least") and multiple candidates tie:
        -   Pick the ONE with **LOWEST employee ID** (alphabetically first)
        -   **Example**: 2 employees with 0.6 workload → pick alphabetically first ID (e.g., BwFV_127 before BwFV_130)
        -   **Example**: 6 employees tied at 0 workload → link ONLY the one with lowest ID (e.g., 6KR2_018)
        -   Do NOT return all tied employees for singular queries!
        -   ⚠️ Do NOT use "more projects" as tie-breaker unless task EXPLICITLY says so!
    -   **"LINK ONLY WINNER" EXCEPTION (t070 fix)**: If task says "link only the winner" or "or none if tied" → when values are EQUAL, return EMPTY entity links!
