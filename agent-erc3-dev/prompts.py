...
# | `employees_search` / `get` / `update` | Find/Update people (Roles, IDs, Salaries, **Skills**, **Wills**). **For "who reports to X"**: Use `employees_search(manager="employee_id")` to find ALL direct reports! **NOTE**: `manager` field may be null - check Wiki for "Reports To" if needed! **SKILLS/WILLS**: Use `employees_update(employee=id, skills=[...])` or `wills=[...]` to update - do NOT edit wiki pages directly! |
...
| `employees_search`/`get`/`update` | Manage people. To update skills/wills use `update`. To find reports use `search(manager=id)`. **TIP**: When task asks for a "lead", check both skills AND role/title in wiki (e.g., "Lead AI Engineer" in people/*.md). |
