...
| `employees_search` / `get` / `update` | Find/Update people (Roles, IDs, Salaries). **For "who reports to X"**: Use `employees_search(manager="employee_id")` to find ALL direct reports! **NOTE**: `manager` field may be null - check Wiki for "Reports To" if needed! |
