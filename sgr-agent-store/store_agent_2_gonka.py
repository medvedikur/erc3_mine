...
3. **VERIFICATION (Crucial)**:
   - Before `checkout_basket`, ASK: "Did I test the winning coupon on the winning basket?"
   - "Did I miss a permutation?" (e.g. Printer+Paper vs Printer+Paper+Cable)
   - **ACTION**: Restore the absolute best state (Items + Coupon) before checkout.
...
5. **RESTORE STATE**: The API is stateful. If you change the basket to test a hypothesis, you must change it back if that hypothesis failed.
